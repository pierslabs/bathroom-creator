...
import { shelfCount, shelfPositions, shelfLayout } from "./shelf";
...
describe("shelfCount", () => {
  ...
  it("nunca negativo: una estantería muy baja no tiene baldas", () => {
...
describe("shelfLayout", () => {
  ...
  it("estantería estándar: 2 baldas, sin divisor (ancho < 0.9)", () => {
    const real: Size = { width: 0.8, height: 0.9, depth: 0.4 };
    const layout = shelfLayout(real, natural);
    expect(layout.shelfYs).toHaveLength(2);
    expect(layout.hasDivider).toBe(false);
    expect(layout.panel).toBeCloseTo(0.02);
  });

  it("estantería ancha: agrega divisor vertical", () => {
    const real: Size = { width: 1.2, height: 0.9, depth: 0.4 };
    expect(shelfLayout(real, natural).hasDivider).toBe(true);
  });

  it("estantería alta: más baldas (cuenta por tamaño real, posiciona en natural)", () => {
